...
public class HtmlTagHandler implements WrapperTagHandler {
    ...
    public static final String H_ITEM = "HTML_TEXTVIEW_ESCAPED_H_TAG";
    public static final String PLACEHOLDER_ITEM = "HTML_TEXTVIEW_ESCAPED_PLACEHOLDER";

    public HtmlTagHandler(BaseActivity baseActivity) {
    ...
    public boolean handleTag(boolean opening, String tag, Editable output, Attributes attributes) {
        if (opening) {
            ...
            } else if (tag.equalsIgnoreCase(H_ITEM)) {
                start(output, new Heading());
                        ...
                        if (lists.peek().equalsIgnoreCase(UNORDERED_LIST)) {
                            ...
                            if (baseActivity.isTablet()) {
                                BulletSpan newBullet = new BulletSpan(indent);
                                        ...
                                        new LeadingMarginSpan.Standard(listItemIndent * (lists.size())),//List.size() +1 will increase space at start
                                        newBullet);
                            } else {
                                BulletSpan newBullet = new BulletSpan(70, ContextCompat.getColor(baseActivity, R.color.black), 8);
                                end(output, Ul.class, false,
                                        new LeadingMarginSpan.Standard(listItemIndent * (lists.size())),//List.size() +1 will increase space at start
                                        newBullet);
                            }
                ...
                final String spannedText = output.subSequence(spanStart, spanEnd).toString();
                    ...
                    public void onClick(View widget) {
                        if (onClickATagListenerProvider != null) {
                            if (getURL().contains("as.me")) {
                                Log.e("url", "Path-- " + getURL());
                                if (HomeHost.Companion.getHomeHost() != null) {
                                    HomeHost.Companion.getHomeHost().consultationTab();
                                    HomeHost.Companion.getHomeHost().runOnUiThread(() -> new Handler().postDelayed(() -> {
                                        //add your code here
                                        HomeHost.Companion.getHomeHost().navigateToConsultation();
                                    }, 100));
                                    ((PackageDetailActivity) baseActivity).finish();
                                }
                            } else {
                                try {
                                    ...
                                    CustomToasts.INSTANCE.showToast(baseActivity, "Url not found!", false);
                                    e.printStackTrace();
                                }
                            }
                        }
...
//                if (!baseActivity.isTablet()) {
                    Typeface myTypeface = ResourcesCompat.getFont(baseActivity, R.font.grotesk_pro_bold);
                    output.setSpan(new RelativeSizeSpan(1.5f), 0, output.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                    end(output, Heading.class, false, new CustomTypeFaceSpan("GrotesqueFont", myTypeface));
//                }
            }
            else if (tag.equalsIgnoreCase("code")) {
